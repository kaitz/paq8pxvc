    /* paq8pxvc file compressor/archiver.  Release by Kaido Orav

    Copyright (C) 2008-2022 Matt Mahoney, Serge Osnach, Alexander Ratushnyak,
...
To compress:   c input output config
To decompress: d input output config

For example:

  paq8pxvc c foo foo.pxv config

compresses foo to foo.pxv using config.

  paq8pxvc d foo.pxv foo config
...
paq8pxvc uses a neural network to combine a large number of models.  The
...
public: 
   ...
   // int filetype;
    ...
    int cInputs;
BlockData():y(0), c0(1), c4(0),bpos(0),blpos(0),finfo(-1),bposshift(0),c0shift_bpos(0),mxInputs(0),cInputs(-1) {
    }
~BlockData(){ }
};
///////////////////////////// ilog //////////////////////////////
...
inline U32 h2(U32 a, U32 b) { return hash1( a,  b);
}
///////////////////////////// BH ////////////////////////////////
...
ContextMap::ContextMap(U64 m, int c, BlockData& bd,int s3,U8 *nn1,U8 *nn2,int cs4,int rd): C(c&255),  t(m>>6),  cn(0),result(0),x(bd) {
    nn=nn1;                       // statetable
    nn01=nn2;                     
...
VM vm(x);
#define vmx(c,i,f) setcomponent(&vm, c, i,  f)
#define vms( a, b, c, d, e, f, g, h, i, j, k, l, m, o, p, q, r, s, sg,bt) components(&vm, a, b, c, d, e, f, g, h, i, j, k, l, m, o, p, q, r, s, sg,bt)
#define vmi(component, componentIndex,  f, d,  indexOfInputs) initcomponent(&vm, component, componentIndex,  f, d,  indexOfInputs)
...
// output from pxv -p option
 mxA[0].update(x.y);
x.mxInputs[0].ncount=0;
// end of updateComponents
}
int VM::getPrediction( ){
    int p=0;
// output from pxv -p option

x.mxInputs[0].add(stretch(smA[0].pr));
prSize[0]=dsA[0].p();
prSize[1]=dhsA[0].p();
prSize[2]=dhsA[0].p();
prSize[3]=dhsA[0].p();
...
prSize[6]=dhsA[0].p();
prSize[7]=dhsA[0].p();
prSize[8]=uasA[0].p(x.y);
x.mxInputs[0].add(stretch(prSize[0])  );
x.mxInputs[0].add(stretch(prSize[1])  );
x.mxInputs[0].add(stretch(prSize[2])  );
x.mxInputs[0].add(stretch(prSize[3])  );
x.mxInputs[0].add(stretch(prSize[4])  );
x.mxInputs[0].add(stretch(prSize[5])  );
x.mxInputs[0].add(stretch(prSize[6])  );
x.mxInputs[0].add(stretch(prSize[7])  );
x.mxInputs[0].add(stretch(prSize[8])  );
prSize[9]=mxA[0].p();
prSize[10]=apm2A[0].p(prSize[9],x.y);
prSize[11]=lmxA[0].average(&prSize[0]);
prSize[12]=apm2A[1].p(prSize[11],x.y);
prSize[13]=lmxA[1].average(&prSize[0]);
p=prSize[13];
...
public:
 ...
 int pr;   
  ...
  void update()  {
    ...
    model::update(x.y,x.c0,x.bpos,x.c4,p());
    pr=vm.getPrediction( );
  }
};
//////////////////////////// Encoder ////////////////////////////
...
private:
  ...
  int decode() {
    ...
    while (((x1^x2)&0xff000000)==0) {  // pass equal leading bytes of range
      x1<<=8;
      x2=(x2<<8)+255;
      x0=(x0<<8)+(fgetc (archive)&255);  // EOF is OK archive->getc()
    }
    return x.y;
  }
...
int main(int argc, char** argv) {
  ...
  if (argc!=4 || argv[1][0]!='c' && argv[1][0]!='d') {
    printf(
      "paq8pxvc %d file compressor (C)  2022, Matt Mahoney et al.\n"
      "Licensed under GPL, http://www.gnu.org/copyleft/gpl.html\n"
      "\n"
      "To compress:   paq8pxvc c input output \n"
      "To decompress: paq8pxvc d input output \n",VERSION);
    return 1;
  }
  ...
  // Compress
  if (argv[1][0]=='c') {
    // Encode header: version 1, file size
    fseek(in, 0, SEEK_END);
    long size=ftell(in);
    ...
    fprintf(out, "pV%c%c%c%c%c", VERSION, size>>24, size>>16, size>>8, size);
  ...
  // Decompress
    ...
    if (getc(in)!='p' || getc(in)!='V' || getc(in)!=VERSION)
      quit("Not a pxv file");
  ...
  printf("%ld -> %ld in %1.2f sec. using %d MB memory.\n", 
    ftell(in), ftell(out), double(clock()-start)/CLOCKS_PER_SEC, (getPeakMemory()/1000)/1000);
